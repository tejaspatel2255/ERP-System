    ...
    return (
        <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <div>
                    <h1 style={{ fontSize: '1.875rem', fontWeight: 800, color: 'var(--text-main)', letterSpacing: '-0.025em' }}>Customers</h1>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
                {filteredCustomers.map(customer => (
                    <div key={customer._id} className="card" style={{ padding: '1.5rem', border: '1px solid var(--border)', transition: 'transform 0.2s, box-shadow 0.2s' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                ...
                                <div>
                                    <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-main)' }}>{customer.name}</h3>
                                    <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>ID: {customer._id.slice(-6).toUpperCase()}</p>
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: '0.5rem' }}>
                                <button
                                    onClick={() => handleEdit(customer)}
                                    style={{ padding: '0.5rem', borderRadius: '0.375rem', background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}
                                >
                                    <Edit2 size={16} />
                                </button>
                                <button
            ...
            {showForm && (
                <div style={{
                    ...
                    <div style={{
                        backgroundColor: 'white', padding: '2rem', borderRadius: '1rem', width: '500px', maxWidth: '90%',
                        ...
                        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                            ...
                            <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                                ...
                                <button type="submit" style={{ flex: 1, padding: '0.75rem', borderRadius: '0.5rem', border: 'none', background: 'var(--primary)', color: 'white', fontWeight: 600, cursor: 'pointer' }}>{isEditing ? 'Save Changes' : 'Create Customer'}</button>
                            </div>
                        </form>
                    </div>
                </div>
