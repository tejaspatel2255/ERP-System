...
function NewDispatchContent() {
    ...
    return (
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            <h1 style={{ fontSize: '1.875rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '2rem' }}>Create Dispatch Manifest</h1>
            ...
            <div className="card" style={{ padding: '2rem' }}>
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        ...
                        >
                            ...
                            {sales.map(sale => (
                                <option key={sale._id} value={sale._id}>
                                    Order #{sale._id.slice(-6).toUpperCase()} - {sale.customerName} - ₹{sale.totalAmount.toLocaleString()} ({new Date(sale.date).toLocaleDateString()})
                                </option>
                            ))}
                        </select>

                        {selectedOrder && (
                            <div style={{ marginTop: '1rem', padding: '1rem', backgroundColor: '#F9FAFB', borderRadius: '0.5rem', border: '1px solid var(--border)' }}>
                                ...
                                <ul style={{ paddingLeft: '1.5rem', margin: 0, fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                                    <li><strong>Customer:</strong> {selectedOrder.customerName}</li>
                                    <li><strong>Items:</strong> {selectedOrder.items.length} items</li>
                    ...
                    <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                        <Button type="button" variant="outline" onClick={() => router.back()} style={{ flex: 1 }}>Cancel</Button>
                        <Button type="submit" style={{ flex: 1 }}>Create Dispatch</Button>
                    </div>
                </form>
            </div>
        </div>
