...
import { Employee, Attendance } from 'erp-shared';
...
export default function AttendancePage() {
    ...
    const employeeAttendanceMap = employees.map(emp => {
        const record = attendanceList.find(a =>
            (typeof a.employee === 'string' ? a.employee : a.employee._id) === emp._id
        );
        return {
            ...emp,
                    ...
                    <tbody>
                        {employeeAttendanceMap.map(item => (
                            <tr key={item._id} style={{ borderBottom: '1px solid var(--border)' }}>
                                ...
                                <td style={{ padding: '1rem' }}>
                                    {item.attendanceRecord?.checkIn ? new Date(item.attendanceRecord.checkIn).toLocaleTimeString() : '-'}
                                </td>
                                <td style={{ padding: '1rem' }}>
                                    {item.attendanceRecord?.checkOut ? new Date(item.attendanceRecord.checkOut).toLocaleTimeString() : '-'}
                                </td>
                                <td style={{ padding: '1rem', textAlign: 'right' }}>
                                    {!item.attendanceRecord ? (
                                        <button
                                            onClick={() => handleCheckIn(item._id)}
                                            style={{ padding: '0.5rem 1rem', borderRadius: '0.375rem', backgroundColor: '#059669', color: 'white', border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}
                                        >
                                        ...
                                        !item.attendanceRecord.checkOut && (
                                            <button
                                                ...
                                                style={{ padding: '0.5rem 1rem', borderRadius: '0.375rem', backgroundColor: '#DC2626', color: 'white', border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}
                                            >
