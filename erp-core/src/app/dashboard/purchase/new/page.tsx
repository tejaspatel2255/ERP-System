...
import { Product, PurchaseItem } from 'erp-shared';
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 2fr) minmax(0, 1fr)', gap: '2rem', alignItems: 'start' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    <div className="card glass-card">
                        <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-main)' }}>
                    ...
                    <div className="card glass-card" style={{ flex: 1 }}>
                        ...
                        <div style={{ maxHeight: '400px', overflowY: 'auto', border: '1px solid var(--border)', borderRadius: '0.5rem' }}>
                            {filteredProducts.map(p => (
                                <div key={p._id} className="hover:bg-slate-50 transition-colors" style={{
                                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                                    padding: '1rem', borderBottom: '1px solid var(--border-light)', backgroundColor: 'rgba(255,255,255,0.5)'
                                }}>
                                    <div>
                                        <div style={{ fontWeight: 600, color: 'var(--text-main)' }}>{p.name}</div>
                                        <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Current Stock: {p.stock} {p.unit}</div>
                                    </div>

                                    {selectedProduct === p._id ? (
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Cost ({currency})</span>
                                                <input
                                                    type="number"
                                                    value={costPrice}
                                                    onChange={e => setCostPrice(e.target.value)}
                                                    placeholder="0.00"
                                                    style={{ width: '80px', padding: '0.25rem', borderRadius: '0.25rem', border: '1px solid var(--primary)', outline: 'none' }}
                                                    autoFocus
                                                />
                                            </div>
                                            <button
                                            ...
                                            <button
                                                onClick={() => { setSelectedProduct(''); setCostPrice(''); }}
                                                style={{ padding: '0.4rem', color: 'var(--text-muted)', background: 'none', border: 'none', cursor: 'pointer' }}
                                            >
                                                Cancel
                                            </button>
                                        </div>
                                        ...
                                        <button
                                            onClick={() => { setSelectedProduct(p._id); setCostPrice(''); }}
                                            style={{
                                                padding: '0.5rem 1rem', borderRadius: '0.375rem',
                                                border: '1px solid var(--border)', background: 'white', color: 'var(--text-main)', cursor: 'pointer',
                ...
                <div className="card glass-card" style={{ height: 'fit-content', position: 'sticky', top: '2rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem', maxHeight: '50vh', overflowY: 'auto' }}>
                        {cart.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: '3rem 1rem', color: 'var(--text-muted)', border: '2px dashed var(--border-light)', borderRadius: '0.5rem', background: 'rgba(255,255,255,0.3)' }}>
                                <ShoppingBag size={32} style={{ marginBottom: '0.5rem', opacity: 0.5 }} />
                                <p>Order is empty</p>
                                <p style={{ fontSize: '0.8rem' }}>Add items from the list to create a purchase order.</p>
                            </div>
                        ) : (
                            cart.map(item => (
                                <div key={item.product._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '1rem', borderBottom: '1px solid var(--border-light)' }}>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                                            ...
                                            <input
                                                ...
                                                style={{ width: '40px', marginLeft: '0.5rem', padding: '0.1rem', borderRadius: '0.25rem', border: '1px solid var(--border)', textAlign: 'center' }}
                                            />
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                        <span style={{ fontWeight: 600, color: 'var(--text-main)' }}>{formatCurrency(item.total)}</span>
                                        <button
                                            onClick={() => removeFromCart(item.product._id)}
                                            style={{
                                                color: '#EF4444', background: '#FEF2F2', border: 'none', cursor: 'pointer', padding: '0.35rem', borderRadius: '0.375rem',
                    ...
                    <div style={{ borderTop: '2px solid var(--border)', paddingTop: '1.5rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem', alignItems: 'center' }}>
                            <span style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-secondary)' }}>Total Payable</span>
