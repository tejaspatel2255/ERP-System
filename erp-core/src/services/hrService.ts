...
import { Employee, Attendance, Leave } from 'erp-shared';
...
export const HRService = {
    ...
    markCheckIn: async (employeeId: string): Promise<Attendance> => {
        const response = await axios.post(`${API_URL}/hr/attendance/checkin`, { employeeId }, { withCredentials: true });
        return response.data;
    },

    markCheckOut: async (employeeId: string): Promise<Attendance> => {
        const response = await axios.post(`${API_URL}/hr/attendance/checkout`, { employeeId }, { withCredentials: true });
        return response.data;
    },
