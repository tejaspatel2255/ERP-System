...
import { Asset, MaintenanceLog } from 'shared';
...
export default function MaintenanceSchedulePage() {
    ...
    const StatusColumn = ({ status, items }: { status: string; items: MaintenanceLog[] }) => {
        const getStatusColor = (s: string) => {
            if (s === 'Pending')
                return { border: 'border-l-4 border-yellow-400', badge: 'bg-yellow-100 text-yellow-800' };
            if (s === 'In Progress')
                return { border: 'border-l-4 border-blue-500', badge: 'bg-blue-100 text-blue-800' };
                    ...
                    {items.map((log) => (
                        ...
                        >
                            ...
                            >
                                ...
                                {status === 'Pending' && (
                                    <button
                                        onClick={() => updateStatus(log._id, 'In Progress')}
                                        style={{
                                ...
                                {status === 'In Progress' && (
                                    <button
                                        onClick={() => updateStatus(log._id, 'Completed')}
                                        style={{
                                            padding: '0.4rem 0.8rem',
            ...
            >
                <div>
                    ...
                    <p style={{ color: 'var(--text-muted)', marginTop: '0.25rem' }}>
                        Track maintenance tasks and issues.
                    </p>
                </div>
                <button
