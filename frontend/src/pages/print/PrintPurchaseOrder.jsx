        ...
        <div className="mx-auto max-w-4xl">
          <div className="flex justify-between border-b pb-4">
            ...
            <div className="text-right">
              ...
              <p>{p.po_no}</p>
              <p>Date: {String(p.po_date).slice(0, 10)}</p>
              <p>Expected: {String(p.expected_date).slice(0, 10)}</p>
            </div>
          </div>
          <div className="mt-4 text-sm"><p className="font-bold">Vendor: {p.vendor_name}</p></div>
          <table className="mt-6 w-full text-sm">
            <thead>
              <tr className="border-b"><th className="text-left py-2">Item</th><th>Qty</th><th>Unit Price</th><th>Total</th></tr>
            </thead>
            <tbody>
              {items.map((item) => (
          ...
          <div className="mt-6 text-right text-sm font-bold">
            <p className="text-lg text-blue-900">Total Order Value: {formatINR(p.total_amount)}</p>
          </div>
          <div className="mt-6 text-sm whitespace-pre-line">{p.notes || import.meta.env.VITE_PO_TERMS || 'Terms apply.'}</div>
        </div>
      )}
    </div>
  );
